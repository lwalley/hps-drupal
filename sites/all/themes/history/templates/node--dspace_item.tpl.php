...
<article id="article-<?php print $node->nid; ?>" class="<?php print $classes; ?> clearfix"<?php print $attributes; ?>>
  ...
  <?php
    ...
    if ($teaser && count($node->field_dspace_image) > 0) {
      print theme('image_style', array('style_name' => 'medium', 'path' => 'DSpace Item/'.$node->field_dspace_image['und'][0]['filename']));
    } elseif ($teaser) {
      print '<div class="description field">'.render(field_view_field('node', $node, 'field_dspace_description', array('label' => 'hidden')));
      print render(field_view_field('node', $node, 'field_dc_description_abstract', array('label' => 'hidden'))).'</div>';
    }
    ...
    if (!$teaser) {
      ...
      // don't print this on the taxonomy term pages
      ...
      if (isset($node->field_dc_rights_uri['und'][0]['url']) && $node->field_dc_rights_uri['und'][0]['url'] != '') {
        print $node->field_dc_rights_uri['und'][0]['url'].'. ';
      } 
      print '</div>';
